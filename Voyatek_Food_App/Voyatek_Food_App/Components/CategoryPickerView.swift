...
class CategoryPickerView: UIView {
    private let categories = ["All", "Morning Feast", "Sunrise Meal", "Dawn Delicacies"]
  private var selectedCategory: String = ""
    ...
    @objc private func handleTap() {
      ...
      let selectAction = UIAlertAction(title: "Select", style: .default) { [weak self] _ in
          ...
          self?.selectedValueLabel.text = self?.selectedCategory
        self?.animateBorder(to: .lightGray)
          
          if self?.selectedCategory.isEmpty == false {
              self?.selectedValueLabel.textColor = .lightGray
          } else {
              self?.selectedValueLabel.text = "Select category"
              self?.selectedValueLabel.textColor = .placeholderText
          }
      }
